            ...
            <table className='w-full flex flex-col' onWheel={(e) => { e.stopPropagation() }}>
                <thead>
                    <tr className='flex py-3 border-y-2 border-amber-400 border-dashed w-full justify-between text-center font-bold text-amber-300 text-[1vw]'>
                        <th className='w-1/4'>Owner ID</th>
                        <th className='w-1/4'>Owner Name</th>
                        <th className='w-1/4'>Email</th>
                        <th className='w-1/4'>Bank Account</th>
                    </tr>
                </thead>
                <tbody className='flex flex-col w-full text-center text-white'>
                    ...
                    <tr key={o._id || i} className='flex py-[1.5vh] border-b border-blue-400/30 w-full justify-between items-center hover:bg-white/5 transition-colors text-[0.9vw]'>
                        <td className='w-1/4 font-bold uppercase truncate px-2'>#{o._id.toString().substring(0,8)}</td>
                        <td className='w-1/4 flex items-center justify-center gap-[0.5vw]'><img src="/dubai.png" className="w-[1.8vw] h-[1.8vw] rounded-full object-cover"/> {o.name}</td>
                        <td className='w-1/4 truncate px-2' title={o.email}>{o.email}</td>
                        <td className='w-1/4 text-amber-200 font-bold tracking-widest'>{o.accountNumber ? `...${o.accountNumber.slice(-4)}` : 'Pending'}</td>
                    </tr>
                    ))}
                </tbody>
            </table>
