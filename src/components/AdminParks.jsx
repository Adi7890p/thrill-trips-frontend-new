            ...
            <table className='w-full flex flex-col' onWheel={(e) => { e.stopPropagation() }}>
                <thead>
                    <tr className='flex py-3 border-y-2 border-amber-400 border-dashed w-full justify-between text-center font-bold text-amber-300 text-[1vw]'>
                        <th className='w-1/5'>Park ID</th>
                        <th className='w-1/5'>Park Name</th>
                        <th className='w-1/5'>City</th>
                ...
                <tbody className='flex flex-col w-full text-center text-white'>
                    ...
                    <tr key={p._id || i} className='flex py-[1.5vh] border-b border-blue-400/30 w-full justify-between items-center hover:bg-white/5 transition-colors text-[0.9vw]'>
                        <td className='w-1/5 font-bold uppercase truncate px-2'>#{p.pid.toString().substring(0,8)}</td>
                        <td className='w-1/5 font-bold truncate px-2' title={p.pname}>{p.pname}</td>
                        <td className='w-1/5 truncate px-2'>{p.pcity}</td>
                        <td className='w-1/5 text-green-300 font-bold'>₹{p.price}</td>
                        <td className='w-1/5'><span className={`text-white px-[0.5vw] py-[0.1vw] rounded text-[0.8vw] font-bold shadow-md truncate ${p.approved === 'Approved' ? 'bg-green-500' : p.approved === 'Rejected' ? 'bg-red-500' : 'bg-yellow-500'}`}>{p.approved}</span></td>
                    </tr>
                    ))}
                </tbody>
            </table>
